...
export default function Dashboard({
	...
	const tokens =
		tokenBalance &&
		tokenBalance.tokens &&
		tokenBalance.tokens.filter((token) => token.tokenInfo.symbol.length < 6);
	const totalEthValue =
		tokenBalance && tokenBalance.ETH.balance * tokenBalance.ETH.price.rate;
	const totalTokensValue = tokens
		? tokens
				.filter((token) => token.tokenInfo.price)
				.map(
					(token, _) =>
						(token.balance / 10 ** parseInt(token.tokenInfo.decimals)) *
						token.tokenInfo.price.rate
				)
				.reduce((a, b) => a + b)
		: 0;
	const inflowAssetsSum =
		transfers &&
		transfers.result &&
		transfers.result
			.filter(
				(transfer) =>
					...
					transfer.tokenSymbol.length < 6
			)
			.reduce((acc, item) => {
				const { tokenSymbol, value, tokenDecimal } = item;
				acc[tokenSymbol] =
					(acc[tokenSymbol] || 0) + parseInt(value, 10) / 10 ** tokenDecimal;
				return acc;
			}, {});
	const outflowAssetsSum =
		transfers &&
		transfers.result &&
		transfers.result
	...
	const balances =
		...
		tokens.map((token, _) => ({
			...
			).toFixed(6)} ${token.tokenInfo.symbol}`,
		}));
			...
			{!error && macroScore && (
				<Box sx={{ pt: 2 }}>
					<Box sx={{ display: "flex" }}>
						...
						<Chip
							label={macroScore.risk_level.split("_").join(" ")}
							color={colors[macroScore.risk_level]}
						/>
					</Box>
					<Grid container spacing={2} sx={{ py: 2 }}>
						<Grid item xs={4}>
							<ValueCard
								...
								graph={<WebGraph dataset={macroScore.score_ingredients} />}
							/>
						</Grid>
						<Grid item xs={4}>
							<ValueCard
								title="PROBABILITY OF LIQUIDATION"
								value={macroScore.probability_of_liquidation / 100}
							/>
						</Grid>
					</Grid>
				</Box>
